...
@implementation CorporatePayments
...
+ (void)initialize {
    ...
    if (!__baseURL) {
        __baseURL = [NSURL URLWithString:@"https://citimobilechallenge.anypresenceapp.com/corporatepayments/v1"];
    }
...
+ (NSURL *)baseURL {
    ...
    if (!__baseURL) {
        @throw [NSException exceptionWithName:kAPRequestBaseURLException
                                       reason:@"CorporatePayments base URL is not set.  Use [CorporatePayments setBaseURL:(NSURL *)URL] to set base URL."
                                     userInfo:nil];
    }
...
+ (NSDictionary *)httpHeaders {
    ...
    [httpHeaders setObject:@"Bearer {{session.corporate_payments_token}}" forKey:@"Authorization"];
