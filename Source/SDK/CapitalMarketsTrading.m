...
@implementation CapitalMarketsTrading
...
+ (void)initialize {
    ...
    if (!__baseURL) {
        __baseURL = [NSURL URLWithString:@"https://citimobilechallenge.anypresenceapp.com/capitalmarkets/v1"];
    }
...
+ (NSURL *)baseURL {
    ...
    if (!__baseURL) {
        @throw [NSException exceptionWithName:kAPRequestBaseURLException
                                       reason:@"CapitalMarketsTrading base URL is not set.  Use [CapitalMarketsTrading setBaseURL:(NSURL *)URL] to set base URL."
                                     userInfo:nil];
    }
...
+ (NSDictionary *)httpHeaders {
    ...
    [httpHeaders setObject:@"Bearer {{session.capital_markets_trading_token}}" forKey:@"Authorization"];
