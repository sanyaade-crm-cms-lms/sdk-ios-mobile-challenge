...
@interface RetailBankingAccountTests : XCTestCase

@end

@implementation RetailBankingAccountTests
...
- (void)testDataSourceMatchesPayloadValue {
    ...
    NSString *actualDataSource = NSStringFromClass([RetailBankingAccount dataSource]);
    NSString *expectedDataSource = @"RetailBanking";
    
    XCTAssertTrue([actualDataSource isEqualToString:expectedDataSource], @"RetailBankingAccount data source was not set properly.  Expected %@ but got %@", expectedDataSource, actualDataSource);
}
...
- (void)testSettingCertificateForDataSource {
    ...
    XCTAssertNotNil(certificatePath, @"RetailBankingAccount certificate path is not valid.  Check that resource exists in SDKTestApp's bundle resources.");
    
    [[RetailBankingAccount dataSource] setSSLCertificate:certificatePath];
    
    XCTAssertTrue([[RetailBankingAccount dataSource] class] == [RetailBanking class]);
    
    NSData *actualCertData = [RetailBanking certificate];
    NSData *expectedCertData = [NSData dataWithContentsOfFile:certificatePath];
    
    XCTAssertTrue([actualCertData isEqual:expectedCertData], @"RetailBankingAccount's certificate data was not set properly.");
}
