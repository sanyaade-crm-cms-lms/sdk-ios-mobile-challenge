//
//  RetailBankingRewardsCatalogTests.m
//  AnyPresence SDK
//
...
@interface RetailBankingRewardsCatalogTests : XCTestCase

@end

@implementation RetailBankingRewardsCatalogTests
...
- (void)testDataSourceMatchesPayloadValue {
    ...
    NSString *actualDataSource = NSStringFromClass([RetailBankingRewardsCatalog dataSource]);
    NSString *expectedDataSource = @"RetailBanking";
    
    XCTAssertTrue([actualDataSource isEqualToString:expectedDataSource], @"RetailBankingRewardsCatalog data source was not set properly.  Expected %@ but got %@", expectedDataSource, actualDataSource);
}
...
- (void)testSettingCertificateForDataSource {
    ...
    XCTAssertNotNil(certificatePath, @"RetailBankingRewardsCatalog certificate path is not valid.  Check that resource exists in SDKTestApp's bundle resources.");
    
    [[RetailBankingRewardsCatalog dataSource] setSSLCertificate:certificatePath];
    
    XCTAssertTrue([[RetailBankingRewardsCatalog dataSource] class] == [RetailBanking class]);
    
    NSData *actualCertData = [RetailBanking certificate];
    NSData *expectedCertData = [NSData dataWithContentsOfFile:certificatePath];
    
    XCTAssertTrue([actualCertData isEqual:expectedCertData], @"RetailBankingRewardsCatalog's certificate data was not set properly.");
}
